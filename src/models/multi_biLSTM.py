...
logger = logging.getLogger("MultiBiLSTM")


class MultiBiLSTM(BaseModel):
    predictions = list()
    ...
    def train(self):
        ...
        class MultiBLSTM():
            def __init__(self, embedding_dim, max_tok, num_cell=256, num_layers=2):
                self.X = tf.placeholder(tf.float32, [None, max_tok, embedding_dim], name="X")
                self.seq_len = tf.placeholder(tf.int32, [None], name="seq_len")
                self.Y = tf.placeholder(tf.float32, [None, 2], name="Y")
                # self.learning_rate = tf.placeholder(tf.float32, [None], name="lr")  # <- adaptive learning rate
        ...
        model = MultiBLSTM(embedding_dim, max_tok_count, num_cell, num_layers)
