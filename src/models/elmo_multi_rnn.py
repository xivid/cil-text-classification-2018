...
logger = logging.getLogger("ELMo_Multi_RNN")


class ELMoMultiRNN(BaseModel):
    predictions = list()
    ...
    def train(self):
        logger.info("Training ElMoMultiRNN model...")
        ...
        def evaluate_voting():
            logger.info("Evaluating the voting of 9 models with individual accuracies " + str(
                get_voting_individual_accuracies()))
            ...
            for i in range(val_samples):
                ...
                if voted_predictions[i] == val_Y_concatenated[i][0] or voted_predictions[i] == -val_Y_concatenated[i][
                    1]:
                    correct_count += 1
        ...
        def predict_voting(submission_file):
            ...
            with open(submission_file, "w+") as f:
                ...
                for p in voted_predictions:
                    ...
                    sample_no += 1
            self.predictions = [1 if p > 0 else -1 for p in voted_predictions]
            ...
            for batchX, batchY, seq_len in batches:
                ...
                if current_step % eval_every_step == 0:
                    ...
                    if is_better_candidate(current_accuracy):
                        ...
                        logger.info("Voting list updated, evaluation accuracy with ensemble: {}".format(ensemble_accuracy))
                        submission_file = out_dir + "kaggle_%s_accu%f.csv" % (
                        datetime.datetime.now().strftime("%Y%m%d%H%M%S"), ensemble_accuracy)
                        logger.info("Generating submission file: %s" % submission_file)
